...
const Enemy = {
  build(type) {
    ...
    const torso = new THREE.Mesh(new THREE.BoxGeometry(0.5 * s, 0.65 * s, 0.3 * s), bodyMat);
    torso.position.y = 1.05 * s;
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.26 * s, 0.26 * s, 0.26 * s), type === 'elite' || type === 'boss' ? Tex.flat(0x2a2d30) : skin);
    ...
    const helmet = new THREE.Mesh(new THREE.BoxGeometry(0.3 * s, 0.12 * s, 0.3 * s), bodyMat);
    helmet.position.y = 1.69 * s;
    const legL = new THREE.Mesh(new THREE.BoxGeometry(0.16 * s, 0.72 * s, 0.18 * s), Tex.flat(0x33392c));
    legL.position.set(-0.13 * s, 0.36 * s, 0);
    const legR = legL.clone(); legR.position.x = 0.13 * s;
    const armGun = new THREE.Group();
    const arm = new THREE.Mesh(new THREE.BoxGeometry(0.12 * s, 0.12 * s, 0.5 * s), bodyMat);
    arm.position.z = -0.25 * s;
    const gun = new THREE.Mesh(new THREE.BoxGeometry(0.07 * s, 0.1 * s, 0.45 * s), Tex.flat(0x1d201a));
    gun.position.z = -0.55 * s;
    ...
    muzzle.position.z = -0.8 * s;
    armGun.add(arm, gun, muzzle);
    armGun.position.set(0.3 * s, 1.25 * s, 0);
    g.add(torso, head, helmet, legL, legR, armGun);
    if (type === 'boss') {
  ...
  update(e, dt) {
    ...
    const canSee = dist < 55 && World.los(eye, pEye);
    ...
    switch (e.state) {
      ...
      case 'attack': {
        ...
        if (e.telegraphT > 0) {
          // raise-weapon telegraph (0.4 s) — visible wind-up before firing
          e.telegraphT -= dt;
          arm.rotation.x = U.lerp(-0.5, 0, e.telegraphT / TELEGRAPH);
          ...
          e.cooldownT -= dt;
          if (canSee) {
            if (e.cooldownT <= 0 && dist < e.cfg.range) e.telegraphT = TELEGRAPH;
            // positioning
            if (e.cfg.strafes) {
              e.strafeT -= dt;
